...
class DayShiftConstraints(BaseModel):
    """
    Shift-related constraints.
    """ # noqa: E501
    max_count: Optional[Annotated[int, Field(strict=True, ge=0)]] = Field(default=1, description="Maximum number of shifts allowed.")
    __properties: ClassVar[List[str]] = ["max_count"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in DayShiftConstraints) in the input: " + _key)

        _obj = cls.model_validate({
            "max_count": obj.get("max_count") if obj.get("max_count") is not None else 1
        })
        return _obj
